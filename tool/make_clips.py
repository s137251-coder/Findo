...
def frame(scene: Image.Image, centre: tuple[float, float], zoom: float) -> Image.Image:
    ...
    box = (cx - view_w / 2, cy - view_h / 2, cx + view_w / 2, cy + view_h / 2)
    return scene.resize((WIDTH, HEIGHT), Image.LANCZOS, box=box)
...
def build(index: int, spot: int, seconds: float) -> Path:
    ...
    path = OUT / f"level_{index:02d}_spot{spot}.mp4"
    ...
    for i in range(total):
        ...
        image = frame(scene, centre, wide + (2.6 - wide) * k)
        radius = WIDTH * (0.75 - 0.52 * k)
        image = with_ring(image, (WIDTH / 2, HEIGHT / 2), radius, int(200 * k))
...
def main() -> None:
    ...
    for index in [int(n) for n in args.levels.split(",") if n.strip()]:
        ...
        path = build(index, spot, args.seconds)
